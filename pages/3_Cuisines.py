...
#linhas_selecionadas3 = df['Cuisines']
#df3 = df.loc[linhas_selecionadas3, :]

st.sidebar.markdown("""___""")

# Filtro Culinárias Escolhas ===============================================
cuisines_options = st.sidebar.multiselect('Escolha os Tipos de Culinária',['Brazilian','Japanese','Italian'], default='Brazilian')

linhas_selecionadas2 = df['Cuisines'].isin( cuisines_options )
df2 = df.loc[linhas_selecionadas2, :]
...
with st.container():
    ...
    restaurante10_maior_nota = maior_nota_restaurante.head(10)
...
with st.container():
    ...
    with col1:
        st.header('Top 10 Melhores Tipos de Culinárias')
        culinaria_maior_nota = df.loc[:, ['Cuisines','Aggregate rating']].groupby('Cuisines').mean().reset_index().sort_values(by='Aggregate rating',ascending=False)
            
        culinaria_top10 = culinaria_maior_nota.head(10)
    ...
    with col2:
        st.header('Top 10 Piores Tipos de Culinárias')
        ...
        culinaria_top10_menos = culinaria_menor_nota.head(10)
